...
public class PolyService {
	...
	//자식타입 참조변수 = 자식객체 주소
		...
		// Tesla 객체를 참조하는 변수의타입이 Car(부모)이기 때문에
		// Tesla 객체가 Car(부모) 객체로 변화
		// -> 업 캐스팅
		
		Car car3 = new Spark(); 
		// Spark 객체를 참조한는 변수의 타입이 Car 
		// Spark 객체가 Car 객체로 변화함
		// -> 업 캐스팅
		
		//1) 자식 객체가 부모객체로 변하였기 때문에 
		//	자신만의 고유한 필드 , 메서드를 사용할 수 없다.
		...
		arr[1] = car2; // Teasla 주소 
		
		arr[2] = car3; // Spark 주소 
		
		
		for(int i = 0 ; i < arr.length ; i++) {
			System.out.print(i +"번째 인덱스의 엔진 : " + arr[i].getEngine());
	...
	public void printCar(Car temp) {
		...
		// 부모타입 참조변수 = 자식타입객체
		// -> 다형성 업캐스팅 모양과 똑같다
		// -> temp라는 Car타입 매개변수에는 Car, Tesla, SparK 주소가 참조되어도 된다.
	...
	System.out.println(arr[1] instanceof Spark); // flase (Spark와 Tesla 관계없음)
	...
	//업 캐스팅 상태(Tesla 자식 -> Car 부모) ; 
	
	System.out.println(((Tesla)c1).getBatteryCapacity() );
	//주의 !
	// " . " 연산자 ( Tesla ) 형변환 하는 연산자보다 우선순위 높음
	...
	public void ex5() {
		...
		//c1.kh.poly.ex1.model.dto.Car.toSting()메소드 코드를 연결
		//정적 바이딩
