...
// Book클래스를 이용하여 책을 추가,수정,삭제,조회하는 메서드 생성

public class BookService {
	
	//  사용자 입력을 받을 Scanner 생
	...
	//사용자가 선택하는 도서관리 프로그램 메뉴 메서드
	public void displayMenu() {
		
		try {
			
			int menuNum = 0; 
			//사용자가 입력할 메뉴 번호 변수 초기화 
			...
			do {
				...
				*/
				...
				//case 외에 숫자를 입력한 경우 해당 값이 출력 
				default : System.out.println("메뉴에 있는 번호만 입력하세요!"); break;
				}
				
				
				
			}while(menuNum != 0); //0이 입력되기 전까지 반복실행 
	...
	public void showBookList(List<Book> list) {
		...
		if(list.isEmpty()) { //list 가 비어있는경우 도서를 등록해달라는 메세지 출력 
